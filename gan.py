...
for epoch in range(opt.n_epochs):
    for i, (imgs, _) in enumerate(dataloader):
        ...
        # Configure input
        real_img = torch.tensor(imgs).cuda()
        ...
        z = torch.randn(imgs.size(0), opt.latent_dim).cuda()
        ...
        if batches_done % opt.sample_interval == 0:
            save_image(gen_imgs.data[:25], "images/"+opt.name+"/%d.png" % batches_done, nrow=5, normalize=True)
            img_grid = make_grid(gen_imgs.data[:25])
            writer.add_image('gan', img_grid, global_step=batches_done)
